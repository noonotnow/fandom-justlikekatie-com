...
export const ActorPreflightLab: React.FC = () => {
  ...
  const [releaseInventory,setReleaseInventory] = useState<AnyRecord|null>(null);
  useEffect(() => { let live=true; api().then(result => { if(live) { const next = result.actors ?? []; setActors(next); setReleaseInventory(result.releaseInventory ?? null); if(next[0]) { setActorId(next[0].actorId); setVibeKey(next[0].pairings?.[0]?.vibeKey ?? ''); } } }).catch(e=>live&&setNotice(e.message)).finally(()=>live&&setLoading(false)); return()=>{live=false}; },[]);
  ...
  function applyRefresh(result:AnyRecord) { if ('calibrationProfile' in result) setCalibrationProfile(result.calibrationProfile ?? null); if ('releaseInventory' in result) setReleaseInventory(result.releaseInventory ?? null); if (result.actors) setActors(result.actors); else if (result.actor) setActors(current => current.map(item => item.actorId === result.actor.actorId ? result.actor : item)); }
  async function refreshReleaseInventory() { try { const result=await api(); applyRefresh(result); } catch(e:any) { setNotice(`The decision was saved, but release inventory could not refresh: ${e.message}`); } }
  ...
  return <section className={styles.lab} aria-labelledby="actor-preflight-title">
    ...
    {releaseInventory&&<ReleaseInventory inventory={releaseInventory}/>}
  ...
  return <section className={styles.releaseInventory} aria-labelledby="release-inventory-title">
    ...
    <div className={styles.inventoryPacks}>{actorPacks.map(pack=><article className={styles.inventoryPack} data-empty={!pack.releaseReadyPairingCount} key={pack.actorId}>
      ...
      <p>{pack.freshCuratorPairingCount??0} fresh curator · {pack.rescueBackupPairingCount??0} rescue-backed pairing{pack.rescueBackupPairingCount===1?'':'s'} · {pack.rescueBackupBoardCount??0} backup board{pack.rescueBackupBoardCount===1?'':'s'}</p>
       {pack.pairings?.length?<><div className={pack.recentlyUsed?styles.actorRepeatWarning:styles.actorLastDrop} role={pack.recentlyUsed?'status':undefined}><strong>{pack.recentlyUsed?'Actor repeat watch':'Last actor Daily Drop'}</strong><span>{pack.lastDailyDropDate?`Last Daily Drop · ${formatEditionDate(pack.lastDailyDropDate)}${pack.recentlyUsed&&pack.recentDailyDropCount>1?` · ${pack.recentDailyDropCount} recent appearances`:''}`:'No recorded Daily Drop'}</span></div><ul>{pack.pairings.map((pair:AnyRecord)=><li key={pair.vibeKey}><span>{pair.vibeLabel}</span><div>{pair.freshCurator&&<em data-kind="fresh">Fresh curator</em>}{pair.rescueBackupBoardCount>0&&<em data-kind="rescue">{pair.rescueBackupBoardCount} rescue backup{pair.rescueBackupBoardCount===1?'':'s'}</em>}{pair.recentlyUsed?<em data-kind="recent">Pair repeat · {formatEditionDate(pair.lastDailyDropDate)}{pair.recentDailyDropCount>1?` · ${pair.recentDailyDropCount} appearances`:''}</em>:<em data-kind="unused">Unused · last {recentWindowDays} days</em>}</div>{pair.recentlyUsed&&<small className={styles.recentUseDates}>Pair appeared {pair.recentDailyDropDates.map(formatEditionDate).join(' · ')}</small>}</li>)}</ul></>:<small>No current release-ready pairings in this actor pack.</small>}
    </article>)}</div>
  </section>;
}
