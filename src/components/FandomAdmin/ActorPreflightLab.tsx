...
function RunEvidence({
  ...
  return <article id="actor-audit-evidence" className={`${styles.card} ${styles.cardWide} ${isLegacy?styles.legacyCard:''}`}>
    ...
    {run ? <>
      ...
      {evidenceAvailable && <><BlindVisualJudgments run={run} isCurrent={isCurrent&&!isLegacy} busy={busy} onSave={onVisualJudgment}/><div className={styles.evidenceSummary}><strong>{displayableCount}</strong><span>displayable retained images</span><strong>{proposedCardCount}</strong><span>complete proposal cards</span><strong>{typeof run.displayCount === 'number' ? run.displayCount : 'Unavailable'}</strong><span>automatically publication-ready cards</span><strong>{typeof run.queryCount === 'number' ? run.queryCount : 'Unavailable'}</strong><span>queries audited</span><strong>{rawResults.length}</strong><span>retained results</span></div><CandidateFunnelSummary run={run}/><CalibrationLearningSummary run={run}/><RequestedGridReview run={run} isCurrent={isCurrent} busy={busy} onSave={onSaveRescue} onExport={onExportRescue} onMarkCalibration={onMarkCalibration} onRetireCalibration={onRetireCalibration} initialReceiptId={initialReceiptId}/><div className={styles.evidence} key={run.runId}>{sections.map(([label,value])=><details key={label}><summary>{label} <span className={styles.muted}>{label === 'Bounded raw results' ? `${isCurrent&&!isLegacy?'Current · writable':isLegacy&&isCurrent?'Legacy · annotations only':isLegacy?'Legacy · frozen read-only':'Retained · frozen read-only'} · ` : ''}{Array.isArray(value)?`${value.length} records`:''}</span></summary>{label === 'Bounded raw results' && rawResults.length > 0 ? <RawResultGrid run={run} isCurrent={isCurrent} isLegacy={isLegacy} busy={busy} onFlag={onFlag} onMisprint={onMisprint}/> : <pre>{text(value)}</pre>}</details>)}</div></>}
