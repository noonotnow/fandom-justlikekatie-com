...
export const ActorPreflightLab: React.FC = () => {
  ...
  async function saveVerdict(event:React.FormEvent) { event.preventDefault(); if(!currentRun?.runId||run?.runId!==currentRun.runId||!verdict)return; setBusy('verdict'); setNotice(''); try { const approved=verdict==='approved'; const result=await api({action:'verdict',actorId,vibeKey,runId:currentRun.runId,verdict,notes,vibeConfirmed:approved&&vibeConfirmed,publishableConfirmed:approved&&publishableConfirmed,rescuePreferred:approved&&rescuePreferred,rescueReceiptId:approved&&rescuePreferred?preferredRescueReceiptId:undefined}); applyRefresh(result); const next=result.currentRun ?? currentRun; const preference=next?.operatorVerdict?.rescuePreference; setRun(next); setCurrentRun(next); setVerdict(result.verdict ?? verdict); setNotes(result.notes ?? notes); setRescuePreferred(preference?.preferred === true); setPreferredRescueReceiptId(preference?.rescueReceiptId ?? ''); setNotice(approved?(preference?.preferred?'Curator result approved as publishable. Your rescue-board preference was recorded separately and does not replace the system winner.':'Curator result approved as publishable with both human confirmations.'):'Verdict saved to the curation ledger.'); } catch(e:any){setNotice(e.message)} finally{setBusy('')} }
       ...
       <main className={styles.detail}>{!actor?<div className={styles.empty}>No actor profiles returned.</div>:<><section className={`${styles.panel} ${styles.detailPanel}`}><div className={styles.detailHead}><div><p className={styles.eyebrow}>Selected profile</p><h4>{actor.canonicalName}</h4><p>{actor.romanizedName} · aliases: {text(actor.aliases)}</p></div><span className={styles.muted}>Profile v{actor.profileVersion ?? '—'}</span></div><div className={styles.pairingStrip}>{(actor.pairings??[]).map(item=><button className={styles.pairing} data-selected={item.vibeKey===vibeKey} key={item.vibeKey} onClick={()=>setVibeKey(item.vibeKey)}><strong>{text(item.labels) || item.vibeKey}</strong><span className={styles.state} data-state={item.auditState}>{item.auditState==='needs_reapproval' ? 'Needs reapproval' : item.auditState==='calibration_reaudit_required' ? 'Calibration reaudit required' : item.verdict ?? item.auditState ?? 'unreviewed'}</span><small>{item.queryCount ?? 0} queries · {date(item.lastRunAt)}</small></button>)}</div><div className={styles.controls}><button className={`${styles.buttonPrimary} ${currentRunIsLegacy?styles.freshAuditButton:''}`} disabled={!vibeKey||!!busy} onClick={()=>void startAudit(scope)}>{busy ? 'Running evidence pass…' : currentRunIsLegacy ? 'Run fresh audit' : 'Run audit'}</button><select className={styles.select} value={scope} onChange={e=>setScope(e.target.value)} aria-label="Audit scope"><option value="representative">Representative scope</option><option value="full">Full scope</option></select><span className={styles.status}>{pairing?.auditState==='blind_review_pending'?'Calibration pending':pairing?.auditState==='calibration_reaudit_required'?'Calibration reaudit required':pairing?.auditState==='needs_reapproval'?'Fresh audit required':pairing?.eligible===false?'Not eligible for scheduling':'Eligible for review'}</span></div></section>
         <section className={styles.panel}>
          ...
          {verdictAvailable ? <form className={styles.verdict} onSubmit={saveVerdict}>
            ...
            {run?.operatorVerdict?.verdict==='approved'&&<div className={styles.approvalOutcome} data-rescue-preferred={finalizedPreference?.preferred===true}><strong>Publishable curator result</strong><span>{run.winner?.mode ? `System winner: ${run.winner.mode}. ` : ''}{finalizedPreference?.preferred?'A rescue board is preferred separately; it did not replace the winner or determine eligibility.':'No rescue board preference was recorded.'}</span></div>}
            ...
            {verdict === 'approved' && <fieldset className={styles.preferenceChecks}><legend>Separate rescue preference — optional</legend><label><input type="checkbox" checked={rescuePreferred} disabled={!savedRescueBoards.length||Boolean(run?.operatorVerdict)} onChange={event=>{setRescuePreferred(event.target.checked);if(event.target.checked&&!preferredRescueReceiptId)setPreferredRescueReceiptId(savedRescueBoards[0]?.receiptId??'')}} /> I prefer a saved rescue board.</label>{rescuePreferred&&<label className={styles.label}>Preferred rescue receipt<select className={styles.select} value={preferredRescueReceiptId} disabled={Boolean(run?.operatorVerdict)} onChange={event=>setPreferredRescueReceiptId(event.target.value)} required><option value="">Choose a saved board</option>{savedRescueBoards.map((receipt:any)=><option key={receipt.receiptId} value={receipt.receiptId}>{date(receipt.savedAt)} · {String(receipt.receiptId).slice(0,8)}</option>)}</select></label>}<p>{savedRescueBoards.length?'This records your editorial preference only. The curator board can still be approved as publishable; the system winner, safety gates, and Daily Drop eligibility stay unchanged.':'Save an exact nine-card rescue board first if you want to record a separate preference.'}</p></fieldset>}
            ...
            <button className={styles.buttonPrimary} disabled={busy==='verdict'||!verdictOptions.includes(verdict)||(verdict==='approved'&&(!vibeConfirmed||!publishableConfirmed||(rescuePreferred&&!preferredRescueReceiptId)))}>Save scheduling verdict</button>
