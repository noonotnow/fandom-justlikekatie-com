...
// @ts-expect-error Actor packs are shared runtime configuration without a TS declaration.
import { ACTOR_PACKS } from '../../../netlify/functions/lib/actor-packs.js';
...
export const ActorPreflightLab: React.FC = () => {
  ...
  const builtInPreviewCopy = [packVibe?.copy, packVibe?.supportingCopy_en, packVibe?.supportingCopy]
    ...
    try {
      const response = await fetch('/.netlify/functions/publish-preflight-preview', {
        ...
        body: JSON.stringify({
          ...
          ...(customCopy ? { editorialCopy: customCopy } : {}),
        }),
      });
      ...
      if (result?.status !== 'published'
        ...
        || !Array.isArray(result.preview.cards)
        || result.preview.cards.length !== 3
        || typeof result.preview.vibe?.copy !== 'string'
  ...
  async function saveCandidateMisprint(candidate:AnyRecord,reason:MisprintReason,actualIdentity:string,note:string) {
    ...
    try {
      const result=await api({action:'mark_misprint',actorId,vibeKey,runId:currentRun.runId,candidateId:candidate.candidateId,reason,actualIdentity,note});
      ...
      if(!imageUrl||!definition||!receipt?.receiptId)throw new Error('The correction was recorded, but the collectible snapshot was incomplete.');
      ...
      setNotice(correctionSaved?`Curator correction saved, but the collectible could not be added to Collection: ${e.message}`:e.message);
    } finally{setBusy('')}
  }
  ...
  async function approveCalibration(adjustmentType:string,signalFamily:string,direction:string,signalValues:string[]) {
    setBusy('calibration-approval'); setNotice('');
    try {
      const result=await api({action:'approve_rescue_calibration',actorId,vibeKey,adjustmentType,signalFamily,direction,signalValues});
      applyRefresh(result); setCalibrationProfile(result.calibrationProfile??null);
      setNotice('Production calibration approved with an immutable aggregate-evidence receipt. Only the selected adjustment will apply to fresh audits.');
    } catch(e:any){setNotice(e.message)} finally{setBusy('')}
  }
  async function revokeCalibrationApproval(approvalId:string,reason:string) {
    setBusy('calibration-revocation'); setNotice('');
    try {
      const result=await api({action:'revoke_rescue_calibration_approval',actorId,vibeKey,approvalId,reason});
      applyRefresh(result); setCalibrationProfile(result.calibrationProfile??null);
      setNotice('Production calibration revoked. Evidence remains in the audit ledger, but no adjustment will apply to future audits.');
      return true;
       ...
       <main className={styles.detail}>{!actor?<div className={styles.empty}>No actor profiles returned.</div>:<><section className={`${styles.panel} ${styles.detailPanel}`}><div className={styles.detailHead}><div><p className={styles.eyebrow}>Selected profile</p><h4>{actor.canonicalName}</h4><p>{actor.romanizedName} · aliases: {text(actor.aliases)}</p></div><span className={styles.muted}>Profile v{actor.profileVersion ?? '—'}</span></div><div className={styles.pairingStrip}>{(actor.pairings??[]).map(item=><button className={styles.pairing} data-selected={item.vibeKey===vibeKey} key={item.vibeKey} onClick={()=>{clearHandoff();setVibeKey(item.vibeKey)}}><strong>{text(item.labels) || item.vibeKey}</strong><span className={styles.state} data-state={item.auditState}>{item.auditState==='needs_reapproval' ? 'Needs reapproval' : item.auditState==='calibration_reaudit_required' ? 'Calibration reaudit required' : item.verdict ?? item.auditState ?? 'unreviewed'}</span><small>{item.queryCount ?? 0} queries · {date(item.lastRunAt)}</small></button>)}</div><div className={styles.controls}><button className={`${styles.buttonPrimary} ${currentRunIsLegacy?styles.freshAuditButton:''}`} disabled={!vibeKey||!!busy} onClick={()=>void startAudit(scope)}>{busy ? 'Running evidence pass…' : currentRunIsLegacy ? 'Run fresh audit' : 'Run audit'}</button><select className={styles.select} value={scope} onChange={e=>setScope(e.target.value)} aria-label="Audit scope"><option value="representative">Representative scope</option><option value="full">Full scope</option></select><span className={styles.status}>{pairing?.auditState==='blind_review_pending'?'Calibration pending':pairing?.auditState==='calibration_reaudit_required'?'Calibration reaudit required':pairing?.auditState==='needs_reapproval'?'Fresh audit required':pairing?.eligible===false?'Not eligible for scheduling':'Eligible for review'}</span></div></section>
          ...
          <div className={styles.grid}>
            ...
            <RunEvidence
              ...
              priorRuns={priorRuns}
              selectedReadOnly={handoffReadOnly}
              busy={busy}
...
function AuditRunPicker({run,currentRun,priorRuns,onSelect}:{run:Run|null;currentRun:Run;priorRuns:Run[];onSelect:(run:Run)=>void}) {
  const runs=[currentRun,...priorRuns];
  const label=(item:Run,index:number)=>`${item.auditContract?.isLegacy?'Legacy history':index===0?'Current':'Retained'} · ${item.runId} · ${date(item.completedAt)}`;
  return <label className={styles.label}>Audit run<select className={styles.select} value={run?.runId ?? ''} onChange={event=>{const selected=runs.find(item=>item.runId===event.target.value);if(selected)onSelect(selected)}}>{runs.map((item,index)=><option key={item.runId} value={item.runId}>{label(item,index)}</option>)}</select></label>;
}
...
function CandidateFunnelSummary({run}:{run:Run}) {
  ...
  return <section className={styles.calibrationLearning} aria-label="Candidate loss funnel">
    ...
    <details><summary>Search cache provenance <span className={styles.muted}>{run.queryRuns?.length ?? 0} receipts</span></summary>
      <p>Diagnostic only. Receipts distinguish a requested or locally applied bypass from a provider-confirmed uncached response. Unknown cache telemetry is never presented as a miss or proof of bypass.</p>
      <pre>{text((run.queryRuns ?? []).map((queryRun:AnyRecord)=>({
        query:queryRun.query,
        cache:queryRun.cacheProvenance,
        resultFingerprint:queryRun.resultFingerprint,
        providerSelectionOrder:queryRun.providerSelectionOrder,
        providerFetchOrder:queryRun.providerFetchOrder,
      })))}</pre>
    </details>
