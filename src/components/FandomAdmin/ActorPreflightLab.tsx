...
export const ActorPreflightLab: React.FC = () => {
  ...
  const verdictAvailable = selectedIsCurrent && (review?.status === 'unavailable' || (review?.choice && !disagreementNeedsReasons));
...
function RunEvidence({
  ...
  return <article className={`${styles.card} ${styles.cardWide}`}>
    ...
    {run ? <>
      ...
      {revealed && <><div className={styles.evidenceSummary}><strong>{run.displayCount ?? 0}</strong><span>clean display images</span><strong>{run.queryCount ?? run.queryRuns?.length ?? 0}</strong><span>queries audited</span><strong>{rawResults.length}</strong><span>retained results</span></div><div className={styles.evidence}>{sections.map(([label,value])=><details key={label}><summary>{label} <span className={styles.muted}>{Array.isArray(value)?`${value.length} records`:''}</span></summary>{label === 'Bounded raw results' && rawResults.length > 0 ? <div className={styles.resultGrid}>{rawResults.map((item:any,index:number)=><a className={styles.result} href={item.link || item.thumbnail || '#'} target="_blank" rel="noreferrer" key={`${item.link || item.thumbnail || item.title || 'result'}-${index}`}>{item.thumbnail ? <img src={item.thumbnail} alt="" loading="lazy" /> : <span className={styles.resultPlaceholder}>No thumbnail</span>}<span>{item.title || 'Untitled result'}</span><small>{item.source || 'Unknown source'}</small></a>)}</div> : <pre>{text(value)}</pre>}</details>)}</div></>}
