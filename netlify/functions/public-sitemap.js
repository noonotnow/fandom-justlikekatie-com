...
import { releasedPackCatalog } from "./lib/released-pack-catalog.js";
...
export function createPublicSitemapHandler({
  ...
  return async (_request, context) => {
    ...
    if (!releaseCatalog.complete) return { statusCode: 503, headers: { "Content-Type": "application/xml", "Cache-Control": "no-store" }, body: "" };
    ...
    const paths = [...PUBLIC_STATIC_PATHS,
      ...
      ...[...new Set(releaseCatalog.packs.map(pack =>
        `${RELEASED_PACK_PATH}/${releasedPackActorSlug(pack.actor)}/`))],
      ...releaseCatalog.packs.map(pack => new URL(pack.canonical).pathname)];
