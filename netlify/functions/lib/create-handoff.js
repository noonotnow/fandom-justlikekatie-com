...
/**
 * The public CREATE entry point accepts only the active Creator Draft source
...
async function validateAuthorization(req, expectedToken, auth, context) {
  ...
  if (auth) {
    try {
      ...
      if (error?.status === 401) {
        throw new RequestError("Sign in again before sending to CREATE.", 401);
      }
      if (error?.status === 403) {
        throw new RequestError("An admin account is required to send drafts to CREATE.", 403);
      }
