...
Repeated human-versus-proxy evidence is diagnostic only. Production may apply one explicitly approved query-ladder or signal-class adjustment, and eligibility must remain bound to the newest active approval over the current evidence set. Every signal in a bundled adjustment must recur together, with the same direction, in the minimum number of distinct reviewed audits.
