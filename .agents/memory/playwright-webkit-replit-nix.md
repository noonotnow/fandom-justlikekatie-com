...
**How to apply:** On Replit only, bypass Playwright’s incompatible host preflight, launch the bundled MiniBrowser directly with its bundle paths plus Replit’s Nix runtime paths, and retain a real launch probe so genuinely missing binaries or libraries still fail with actionable output.